...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if args.method == 'NC':
            ...
            target_t1_ = nn.Softmax(dim=1)(target_weak)
            smo_loss = -torch.mean(torch.sum(target_t1_*torch.log(target_t1_+1e-8),dim=1))/torch.log(torch.tensor(target_t1_.shape[1]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lamda', default=1, type=float,help='lamda for smoothness loss')
